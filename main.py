...
def question_2(df1):
    ...
    def avg_lat(df):
        ...
        for i in range(length):
            ...
            f = json.loads(j)
            latitude = f['Latitude']
            sum_lat.append(latitude)
            longitude = f['Longitude']
            sum_lon.append(longitude)
        sum_la ={}.fromkeys(sum_lat).keys()
        sum_lo ={}.fromkeys(sum_lon).keys()
    ...
    df1["avg_longitude"] = df1["avg_latitude"].map(lambda x:x.split("|")[1])
    df1["avg_latitude"] =df1["avg_latitude"].map(lambda x:x.split("|")[0])
    df2 = df1
    #################################################
...
def question_4(df2, continents):
    ...
    df2["Covid_19_Economic_exposure_index"] = df2["Covid_19_Economic_exposure_index"].apply(
        lambda x: x.replace(',', '.'))
...
def question_5(df2):
    ...
    """
    #################################################
    df = pd.DataFrame(df2, columns=["Income classification according to WB", "Foreign direct investment",
...
def question_9(df2):
    ...
    #################################################
    df = pd.DataFrame(df2, columns=["Income classification according to WB",
    ...
    df["Covid_19_Economic_exposure_index_Ex_aid_and_FDI_and_food_import"] = df[
        "Covid_19_Economic_exposure_index_Ex_aid_and_FDI_and_food_import"].apply(lambda x: x.replace(',', '.'))
    df["Covid_19_Economic_exposure_index_Ex_aid_and_FDI"] = df["Covid_19_Economic_exposure_index_Ex_aid_and_FDI"].apply(
        lambda x: x.replace(',', '.'))
    df["Foreign direct investment, net inflows percent of GDP"] = df[
        "Foreign direct investment, net inflows percent of GDP"].apply(lambda x: x.replace(',', '.'))
    df["Foreign direct investment"] = df["Foreign direct investment"].apply(lambda x: x.replace(',', '.'))
    ...
    df[["Covid_19_Economic_exposure_index_Ex_aid_and_FDI",
        ...
        "Foreign direct investment, net inflows percent of GDP"]] = df[
        ["Covid_19_Economic_exposure_index_Ex_aid_and_FDI",
         "Covid_19_Economic_exposure_index_Ex_aid_and_FDI_and_food_import", "Foreign direct investment",
...
def question_10(df2, continents):
    ...
    plt.legend([p1, p2, p3, p4, p5, p6], ["Africa", "Asia", "Europe", "North America", "Oceania", "South America"],
               loc='best', markerscale=0.7, fontsize=20)
